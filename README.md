...
- v2: モバイルファースト UI、ニュースセンチメント、サプライチェーンマップ、銘柄シート(テクニカル分析)、
  週間解説・テーマ騰落サマリー・イベントカレンダー・マクロ指標パネルを追加
- UI: v1 のダークトンマナ(レインボーバー・上昇=赤/下落=青)を踏襲。フォントは Zen Kaku Gothic New / Shippori Mincho / IBM Plex Mono
...
| タブ | パス | 内容 |
|------|------|------|
| ホーム | `/` | 今週の解説、テーマ騰落サマリー(前日比・YTD・SOX比α)、2週間イベント、米日連動トリガー、SOX指数、マクロ指標(WSTS・CapEx)、週間センチメント(ウェハー)、注目分析3本、最新ヘッドライン、AIサマリーコピー |
| ニュース | `/news` | 自動ヘッドライン(13媒体)、イベントカレンダー(2週間・詳細パネル)、センチメント定点観測(全体+12テーマ切替)、分析ニュース一覧(フィルタ付)、地政学タイムライン |
| テーマ | `/themes` | v1 トラッカー相当 — 7ビュー(テーマ/銘柄分析/前日比・出来高/騰落率/フロー図/連動/工程)、銘柄検索・タグ検索・銘柄シート |
| マップ | `/map` | サプライチェーンマップ(4ステージ) — 装置\|材料の対比、今週の論点・ニュース件数 |
| 学ぶ | `/learn` | 用語辞典(HBM, CoWoS, EUV 等) — 「なぜ株価に効くか」まで解説、関連テーマへのリンク |

### ホーム画面(上から順)

1. **今週の解説** — `weekly_digest.json` の手動エディトリアル
2. **テーマ騰落サマリー** — 12マクロの前日比・年初来・SOX比(α)、スパークライン・出来高シグナル
3. **今後2週間のイベント** — 決算・規制・業界イベント(プレビュー)
4. **米日連動トリガー** — 前日 +2% 超の米国テーマがあればバナー表示
5. **SOX指数** — 終値・前日比・年初来
6. **マクロ指標** — WSTS 月次売上・サイクルコメント・主要 CapEx ガイダンス
7. **今週のセンチメント** — ウェハーマップ + 週間スコア
8. **今週の注目3本** / **最新ヘッドライン**
9. **AI分析用サマリーコピー**

**モバイル**: ヘッダー右のハンバーガーメニューから上記セクションへアンカージャンプ(`md` 未満のみ)。
**イベントカレンダー**: モバイルは日付ごとの縦リスト、タブレット以上は7列グリッド。
...
| `weekly_digest.json` | ホーム最上部の「今週の解説」(週次タイトル・本文) |
| `events.json` | イベントカレンダー(決算/規制/業界/マクロ、テーマ tags 紐付け) |
| `macro.json` | マクロ指標(WSTS 月次売上・サイクルコメント・CapEx ガイダンス) |
| `supplychain.json` | サプライチェーンマップ(4ステージ構成・今週の論点 topic) |
| `timeline.json` | 地政学タイムライン(規制イベント等) |
| `glossary.json` | 用語辞典(関連テーマ tags 付き) |
...
- **センチメント定点観測**(ニュースタブ): 月曜起点の週次平均スコア推移(12週)。全体または12マクロテーマを切替

### イベントカレンダー(手動)

`data/events.json` に日付・種別(earnings/regulation/industry/macro)・タイトル・本文を追記。
基準日(`themes_perf.json` の `last_updated`)を含む週の月曜から2週間を表示。ホームはコンパクトプレビュー、
ニュースタブはタップで詳細パネル付きのフル表示。

### マクロ指標(手動)

`data/macro.json` で WSTS 月次売上(十億ドル・前年比)、シリコンサイクル位置づけのコメント、
主要ファブ・メモリの CapEx ガイダンスを更新。自動取得パイプラインは未実装(Phase 2 以降で検討)。
...
- **銘柄検索**: コード・社名の部分一致(例: `6146` / `ディスコ`)
- **タグ検索**: `pipeline/tags.py` 由来のタグで絞り込み(例: `先端パッケージング` / `MLCC` / `PCB`)。銘柄シートからタグをタップすると検索欄にプリフィルされます

### AI分析用サマリーコピー

サイト上のデータをテキスト化してクリップボードにコピーする機能です。

- **ホーム** (`CopySummary`): 今週の解説・週間センチメント・指数・マクロ(WSTS/CapEx)・テーマ騰落(YTD・SOX比α)・米日連動・注目ニュースをまとめたサマリー
- **銘柄分析ビュー** (`TrackerCopySummary`): デイトレ/スイング候補・過熱銘柄・米国トリガー連動をまとめた市場サマリー

Claude 等の AI に貼り付けて質問する用途を想定しています(投資助言ではありません)。
